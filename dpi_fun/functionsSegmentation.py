...
def mainSegmentation(config, logger, avis, imgsets):
    ...
    if not os.path.exists(config['raw_dir']):
        logger.error(
            f'Specified path ({config["raw_dir"]}) does not exist. Stopping.')
        sys.exit(1)
    ...
    if config['segmentation']['use_scratch']:
        try:
            # TODO dir_permissiosn throughout.
            os.makedirs(config['segmentation']['scratch_dir'],
                        mode=config['general']['dir_permissions'],  exist_ok=True)
        except PermissionError:
            # TODO Further exception.
            logger.error(
                f"Permission denied while attempting to make scratch directory: {config['segmentation']['scratch_dir']}.")
    ...
    if len(avis) > 0:
        with concurrent.futures.ProcessPoolExecutor(max_workers=num_threads) as executor:
            future_to_file = {executor.submit(
                process_video, segmentation_dir, config, filename): filename for filename in avis}
            ...
            for future in concurrent.futures.as_completed(future_to_file):
                ...
                try:
                    ...
                    logger.error(
                        f'Processing {filename} generated an exception: {exc}')
                else:
                    end_time = time()
                    logger.info(
                        f"Processed {n} of {len(avis)} files.\t\t Estimated remainder: {(end_time - init_time)/n*(len(avis)-n) / 60:.1f} minutes.\t Elapsed time: {(end_time - init_time)/60:.1f} minutes.")
                    n += 1

    if len(imgsets) > 0:
        with concurrent.futures.ProcessPoolExecutor(max_workers=num_threads) as executor:
            future_to_file = {executor.submit(
                process_image_dir, folder, segmentation_dir, config): folder for folder in imgsets}
            ...
            for future in concurrent.futures.as_completed(future_to_file):
                ...
                try:
                    ...
                    end_time = time()
                    logger.info(
                        f"Processed {n} of {len(imgsets)} files.\t\t Estimated remainder: {(end_time - init_time)/n*(len(imgsets)-n) / 60:.1f} minutes.\t Elapsed time: {(end_time - init_time)/60:.1f} minutes.")
                    n += 1
    timer['processing_end'] = time()

    timer['archiving_start'] = time()
    if len(avis) > 0 and config['segmentation']['cleanup']:
        # Important to isolate processing and cleanup since the threads don't know when everything is done processing.
        logger.info('Archiving results and cleaning up.')
        with concurrent.futures.ProcessPoolExecutor(max_workers=num_threads) as executor:
            future_to_file = {executor.submit(
                cleanupFile, filename, segmentation_dir, config): filename for filename in avis}
            init_time2 = time()
            n = 1
            for future in concurrent.futures.as_completed(future_to_file):
                filename = future_to_file[future]
                try:
                    ...
                    end_time = time()
                    logger.info(
                        f"Cleaning up {n} of {len(avis)} files.\t\t Estimated remainder: {(end_time - init_time2)/n*(len(avis)-n) / 60:.1f} minutes.\t Elapsed time: {(end_time - init_time2)/60:.1f} minutes.")
                    n += 1
    ...
    if len(imgsets) > 0 and config['segmentation']['cleanup']:
        ...
        with concurrent.futures.ProcessPoolExecutor(max_workers=num_threads) as executor:
            future_to_file = {executor.submit(
                cleanupFile, filename, segmentation_dir, config): filename for filename in imgsets}
            ...
            for future in concurrent.futures.as_completed(future_to_file):
                ...
                try:
                    ...
                    end_time = time()
                    logger.info(
                        f"Cleaning up {n} of {len(imgsets)} files.\t\t Estimated remainder: {(end_time - init_time2)/n*(len(imgsets)-n) / 60:.1f} minutes.\t Elapsed time: {(end_time - init_time2)/60:.1f} minutes.")
                    n += 1
    timer['archiving_end'] = time()
    logger.info(
        f"Finished segmentation. Total time: {(time() - timer['processing_start'])/60:.1f} minutes.")
...
def process_video(segmentation_dir, config, avi_path):
    ...
    if is_file_above_minimum_size(statistics_filepath, 0, logger):
        if config['segmentation']['overwrite']:
            logger.info(
                f"Config file enables overwriting, removing files for {filename}.")
            ...
            logger.info(
                f"Overwritting is not allowed and prior statistics file exists. Skipping {filename}.")
            return
    ...
    if 'csv' in config['general']['export_as']:
        try:
            ...
            logger.error(
                f"Permission denied when making directory {scratch_path}.")

        try:
            os.makedirs(output_path, exist_ok=True)
        except PermissionError:
            logger.error(
                f"Permission denied when making directory {output_path}.")

        if config['segmentation']['diagnostic']:
            diagnostic_path = segmentation_dir + os.path.sep + \
                filename + " diagnostic" + os.path.sep
            try:
                os.makedirs(diagnostic_path, exist_ok=True)
            except PermissionError:
                logger.error(
                    f"Permission denied when making diagnostic directory {diagnostic_path}.")
    ...
    frame_buffer = []
    n = 1  # Absolute input frame counter
    tile_number = 1  # Output tiled frame counter
    ...
    while video.isOpened():
        ...
        if frame is not None:
            ...
            if len(frame_buffer) == config['segmentation']['ntile']:
                # Tile and rotate full tile
                tiled = np.vstack(frame_buffer)
                #rotated = cv2.rotate(tiled, cv2.ROTATE_90_COUNTERCLOCKWISE)

                process_linescan_frame(
                    Frame(
                        sourcePath=avi_path,
                        destPath=scratch_path,
                        filename=filename,
                        frameNumber=tile_number,
                        data=tiled
                    ),
                    config,
                    logger
    ...
    # Handle partial tile at the end
    if frame_buffer:
        tiled = np.vstack(frame_buffer)
        #rotated = cv2.rotate(tiled, cv2.ROTATE_90_COUNTERCLOCKWISE)
...
def process_image_dir(img_path, segmentation_dir, config):
    ...
    if 'csv' in config['general']['export_as']:  # Need to construct some directories
        try:
            os.makedirs(output_path, exist_ok=True)
            logger.debug(
                f"Created directory {output_path} if not already existing.")
        except PermissionError:
            logger.error(
                f"Permission denied: Unable to create output directory '{output_path}'.")
            return
        except OSError as e:
        ...
        try:
            os.makedirs(scratch_path, exist_ok=True)
            logger.debug(
                f"Created directory {scratch_path} if not already existing.")
        except PermissionError:
            logger.error(
                f"Permission denied: Unable to create scratch directory '{scratch_path}'.")
            return
        except OSError as e:
            # Catch any other OS-related errors
        ...
        if config['segmentation']['diagnostic']:
            try:
                ...
                logger.error(
                    f"Permission denied: Unable to create diagnostic directory '{diagnostic_path}'.")
            except OSError as e:
                # Catch any other OS-related errors
                logger.error(
                    f"Error creating directory '{diagnostic_path}': {e}")
    ...
    for f in os.listdir(img_path):
        if f.endswith(valid_extensions):
            ...
            process_area_frame(
                Frame(sourcePath=img_path + os.path.sep,
                      destPath=scratch_path, filename=f, frameNumber=k, data=None),
...
def process_linescan_frame(frame, config, logger):
    ...
    """
    logger.debug(
        f"Pulled frame from queue. Processing {frame.get_filename()}.")

    # Read img and flatfield
    gray = cv2.cvtColor(frame.read(), cv2.COLOR_BGR2GRAY)
    gray = np.array(gray)

    field = np.quantile(gray, q=float(
        config['segmentation']['flatfield_q']), axis=0)
    ...
    if config['segmentation']['diagnostic']:
        ...
        cv2.imwrite(
            f'{destPath[:-1] + " diagnostic" + os.path.sep}{fileName}-{frameNumber:06}-3qualtilefield.jpg', gray.T)
        cv2.imwrite(
            f'{destPath[:-1] + " diagnostic" + os.path.sep}{fileName}-{frameNumber:06}-4annotated.jpg', cv2.transpose(grayAnnotated))
        cv2.imwrite(
            f'{destPath[:-1] + " diagnostic" + os.path.sep}{fileName}-{frameNumber:06}-2threshold.jpg', thresh.T)
        cv2.imwrite(
            f'{destPath[:-1] + " diagnostic" + os.path.sep}{fileName}-{frameNumber:06}-1original.jpg', cv2.transpose(frame.read()))
...
def process_area_frame(frame, config, logger, apply_corrections=True):
    ...
    """
    logger.debug(
        f"Pulled frame from queue. Processing {frame.get_filename()} {frame.get_frame_number()}.")

    # white = target, black = bkg
    image = ~cv2.imread(frame.get_source_path() + frame.get_filename(), 0)
    ...
    if apply_corrections:
        ...
        image = image[(center[1]-radius):(center[1] + radius),
    ...
    with open(f'{destPath[:-1]} statistics.csv', 'a', newline='\n') as outcsv:
        ...
        for i in range(len(cnts)):
            ...
            if 2*w + 2*h >= int(config['segmentation']['min_perimeter_statsonly']):
                newstat = calcStats(
                    gray[y:(y+h), x:(x+w)], cnts[i], x, y, w, h)
                outwritter.writerow([frameNumber, i, *newstat])

    if config['segmentation']['diagnostic']:
        logger.debug(
            f"Diagnostic mode, saving threshold image and quantiledfiled image.")
        # cv2.imwrite(f'{destPath[:-1] + " diagnostic" + os.path.sep}{fileName}-{frameNumber:06}-corrected.jpg', gray)
        cv2.imwrite(
            f'{destPath[:-1] + " diagnostic" + os.path.sep}{fileName}-{frameNumber:06}-2annotated.jpg', grayAnnotated)
        cv2.imwrite(
            f'{destPath[:-1] + " diagnostic" + os.path.sep}{fileName}-{frameNumber:06}-1original.jpg', image)
        # cv2.imwrite(f'{destPath[:-1] + " diagnostic" + os.path.sep}{fileName}-{frameNumber:06}-background.jpg', bkg)
        cv2.imwrite(
            f'{destPath[:-1] + " diagnostic" + os.path.sep}{fileName}-{frameNumber:06}-3threshold.jpg', thresh)
...
def cleanupFile(rawPath, segmentation_dir, config):
    ...
    if is_file_above_minimum_size(segmentation_dir + os.path.sep + filename + '.zip', 0, logger):
        if config['segmentation']['overwrite']:
            logger.warn(
                f"archive exists for {filename} and overwritting is allowed. Deleting old archive.")
            delete_file(segmentation_dir + os.path.sep +
                        filename + '.zip', logger)
        else:
            logger.warn(
                f"archive exists for {filename} and overwritting is allowed. Skipping Archiving")

    shutil.move(scratch_path[:-1] + ' statistics.csv', output_path)  # TODO
    delete_file(scratch_path[:-1] + ' statistics.csv', logger)  # TODO

    shutil.make_archive(segmentation_dir + os.path.sep +
                        filename, 'zip', scratch_path)
...
def constructSegmentationDir(rawPath, config):
    ...
    and raw file path.
    """
    segmentationDir = rawPath.replace(
        "raw", "analysis")  # /media/plankline/Data/analysis/Camera1/Transect1

    for i in range(5):
        # /media/plankline/Data/analysis/Camera1/Transect1
        segmentationDir = segmentationDir.replace(
            f"camera{i}/", f"camera{i}/segmentation/")

    # /media/plankline/Data/analysis/segmentation/Camera1/segmentation/Transect1-REG
    segmentationDir = segmentationDir + \
        f"-{config['segmentation']['basename']}"
    logger = setup_logger('Segmentation (Main)', config)
    logger.debug(f"Segmentation directory: {segmentationDir}")
    try:
        os.makedirs(segmentationDir, int(
            config['general']['dir_permissions']), exist_ok=True)
    except PermissionError:
        logger.error(
            f"Permission denied: Unable to create segmentation directory '{directory_path}'.")
        sys.exit(1)
    except OSError as e:
...
def findVideos(config, logger, checkWriteStatus=False):
    ...
    if checkWriteStatus:
        avis = [os.path.join(raw_dir, avi) for avi in os.listdir(raw_dir) if avi.endswith(
            valid_extensions) and is_file_finished_writing(os.path.join(raw_dir, avi))]
    else:
        avis = [os.path.join(raw_dir, avi) for avi in os.listdir(
            raw_dir) if avi.endswith(valid_extensions)]
...
def findImgsets(config, logger, checkWriteStatus=False):
    ...
    raw_dir = config['raw_dir']
    imgsets = []
    imgsets = [os.path.join(raw_dir, d) for d in os.listdir(
